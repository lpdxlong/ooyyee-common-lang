...
class Http {
     ...
     * @return string
	 */
	private static function request($url, $post = '', $headers = array(), $timeout = 60) {
		$ch = curl_init ();
		$curl_url = $url;
	...
	public static function get($url,$json_decode=true, $headers = array()) {
		$data= self::request ( $url, null, $headers, 60 );
		if($json_decode){
	...
	public static function post($url, $data, $json_decode=true,$headers = array()) {
		...
		$data= self::request ( $url, $data, $headers, 60 );
		if($json_decode){
			$data=@json_decode($data,true);
		}
		return $data;
	 ...
	 * 检查是否是微信浏览器打开
	 */
	public static function checkWxAgent() {
	 ...
	 * 判断企业微信
	 */
	public static function checkWxWorkAgent(){
